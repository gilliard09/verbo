...
import {
  ...
  Users, Star, Quote, Sparkles, ArrowRight, MessageCircle,
  TrendingUp, Clock, Heart, Target, Play, X
} from 'lucide-react';
...
const FAQ = [
  ...
  },
  {
    ...
    resposta: 'Não. Você pode criar sua conta e começar a usar o Verbo gratuitamente agora mesmo. Planos com recursos extras ficam disponíveis dentro do app quando você quiser evoluir.',
  },
];
...
const FaqItem = ({ pergunta, resposta, delay, visivel }: { pergunta: string; resposta: string; delay: number; visivel: boolean }) => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-bold text-slate-800 text-sm leading-snug">{pergunta}</span>
        <ChevronRight
          size={18}
...
// ─── Modal de Vídeo ────────────────────────────────────────────────────────
const VideoModal = ({ open, onClose, videoId }: { open: boolean; onClose: () => void; videoId: string }) => {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full max-w-3xl aspect-video rounded-[24px] overflow-hidden shadow-2xl bg-black"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute -top-2 -right-2 md:top-3 md:right-3 z-10 w-10 h-10 rounded-full bg-white text-slate-800 flex items-center justify-center shadow-xl hover:scale-105 active:scale-95 transition-transform"
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-slate-900 overflow-x-hidden" style={{ fontFamily: "'Poppins', sans-serif" }}>
      <style>{`
        ...
        * { font-family: 'Poppins', sans-serif; }
        ...
        .pulse-play { animation: pulse-play 2.2s ease-in-out infinite; }
      ...
      <VideoModal open={videoAberto} onClose={() => setVideoAberto(false)} videoId={YOUTUBE_VIDEO_ID} />
      ...
      <section className="hero-glow px-6 pt-12 pb-24 max-w-6xl mx-auto text-center relative">
        ...
        <div className="relative z-10">
          ...
          <div className="inline-flex items-center gap-2 bg-white border border-purple-100 px-4 py-2 rounded-full mb-6 shadow-sm animate-slide-up">
            ...
            <span className="text-[11px] font-bold text-slate-700">Mais de <span className="text-[#4C1D95]">{TOTAL_USUARIOS} pregadores</span> já usam o Verbo</span>
          </div>
          ...
          <div className="inline-flex items-center gap-2 bg-purple-50 border border-purple-100 px-4 py-2 rounded-full mb-8 animate-slide-up" style={{ animationDelay: '100ms' }}>
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#4C1D95]">Tecnologia que serve à Palavra</span>
          </div>
          ...
          <div className="mt-8 flex items-center justify-center gap-2 text-xs text-gray-400 font-medium animate-slide-up" style={{ animationDelay: '500ms' }}>
            ...
            <span className="font-bold text-slate-600">Mais de <span className="text-[#4C1D95]">{TOTAL_SERMOES} sermões</span> já criados</span>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-16 bg-gradient-to-b from-white to-slate-50" data-section="beneficios">
        <div className="max-w-5xl mx-auto">
          <div {...anim('beneficios', 0)} className="text-center mb-12">
            <span className="text-[#4C1D95] font-bold text-[10px] uppercase tracking-[0.2em] mb-3 block">Por que usar o Verbo</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {BENEFICIOS.map((b, i) => (
              <div key={i} {...anim('beneficios', 100 + i * 80)}
                ...
                <h3 className="font-bold text-slate-800 mb-2 text-base">{b.titulo}</h3>
      ...
      {/* ── DEPOIMENTOS REAIS (movido para logo após benefícios) ── */}
      <section className="px-6 py-24 bg-white" data-section="depoimentos">
        <div className="max-w-5xl mx-auto">
          <div {...anim('depoimentos', 0)} className="text-center mb-14">
            <span className="text-[#4C1D95] font-bold text-[10px] uppercase tracking-[0.2em] mb-3 block">Prova Social</span>
          ...
          <div {...anim('depoimentos', 500)} className="text-center">
            <div className="inline-flex items-center gap-3 bg-gradient-to-r from-purple-50 to-white border border-purple-100 px-6 py-4 rounded-[20px] shadow-sm">
              ...
              <p className="font-bold text-[#4C1D95] text-sm">{TOTAL_USUARIOS}+ pregadores • {TOTAL_SERMOES}+ sermões criados</p>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* ── DOR vs SOLUÇÃO ── */}
      <section className="px-6 py-20 bg-gradient-to-b from-slate-50 to-white" data-section="dor">
        <div className="max-w-3xl mx-auto">
          ...
          <div {...anim('dor', 500)} className="text-center">
            <div className="inline-block bg-gradient-to-r from-[#4C1D95] to-[#5B21B6] text-white px-10 py-5 rounded-3xl font-bold text-xl shadow-2xl shadow-purple-200">
      ...
      <section className="px-6 py-24 max-w-5xl mx-auto" data-section="features">
        <div {...anim('features', 0)} className="text-center mb-14">
          <span className="text-[#4C1D95] font-bold text-[10px] uppercase tracking-[0.2em] mb-3 block">O que você encontra no app</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FEATURES.map((f, i) => (
            <div key={i} {...anim('features', 100 + i * 80)}
              ...
              {(f as any).novo && (
                <div className="absolute -top-2.5 left-5 bg-gradient-to-r from-[#4C1D95] to-[#5B21B6] text-white px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-widest flex items-center gap-1 shadow-lg">
              ...
              <h3 className="font-bold text-slate-800 mb-2 text-sm tracking-tight">{f.titulo}</h3>
      ...
      <section className="px-6 py-24 bg-gradient-to-b from-white to-slate-50" data-section="screenshots">
        <div className="max-w-5xl mx-auto">
          <div {...anim('screenshots', 0)} className="text-center mb-14">
            <span className="text-[#4C1D95] font-bold text-[10px] uppercase tracking-[0.2em] mb-3 block">Veja o app por dentro</span>
            ...
            {SCREENSHOTS.map((s, i) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-full text-xs font-bold uppercase tracking-wide transition-all ${
                    screenshotAtivo === i
      ...
      {/* ── VÍDEO (thumbnail com play overlay → abre modal) ── */}
      <section className="px-6 py-24 max-w-4xl mx-auto bg-white" data-section="video">
        <div {...anim('video', 0)} className="text-center mb-8">
          <span className="text-[#4C1D95] font-bold text-[10px] uppercase tracking-[0.2em]">Veja como funciona</span>
          <h2 className="text-3xl font-black tracking-tight mt-2">Em alguns minutos você entende tudo.</h2>
        </div>
        <button
          {...anim('video', 150)}
          onClick={() => setVideoAberto(true)}
          className="group relative block w-full rounded-[32px] overflow-hidden shadow-2xl border-8 border-white bg-slate-900 aspect-video cursor-pointer"
        ...
        >
          ...
          <div
            className="pulse-play absolute top-1/2 left-1/2 w-20 h-20 rounded-full bg-white/95 flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform"
          >
      ...
      <section className="px-6 py-24 bg-gradient-to-b from-slate-50 to-white" data-section="autor">
          ...
          <div {...anim('autor', 0)} className="flex justify-center">
            <div className="relative">
              ...
              <div className="absolute -bottom-4 -right-4 bg-gradient-to-br from-[#4C1D95] to-[#5B21B6] text-white px-5 py-3 rounded-2xl shadow-xl">
                ...
                <p className="text-sm font-bold">Jeferson Rocha</p>
              </div>
            </div>
          </div>
          <div {...anim('autor', 150)}>
            <span className="text-[#4C1D95] font-bold text-[10px] uppercase tracking-[0.2em] mb-4 block">Quem criou o Verbo</span>
      ...
      <section className="px-6 py-24 bg-white" data-section="faq">
        <div className="max-w-2xl mx-auto">
          <div {...anim('faq', 0)} className="text-center mb-12">
            <span className="text-[#4C1D95] font-bold text-[10px] uppercase tracking-[0.2em] mb-3 block">Dúvidas frequentes</span>
      ...
      <section className="px-6 py-20" data-section="cta">
        <div {...anim('cta', 0)} className="max-w-3xl mx-auto bg-gradient-to-br from-[#4C1D95] via-[#5B21B6] to-[#6D28D9] rounded-[48px] p-12 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10">
            ...
            <div className="flex justify-center gap-5 mt-6 text-[10px] font-bold text-purple-300 uppercase tracking-widest">
      ...
      <footer className="py-12 text-center border-t border-gray-100 bg-white">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <p className="text-gray-400 text-[10px] font-bold uppercase tracking-[0.3em] mb-3">
          ...
          <button
            ...
            className="text-gray-400 text-[11px] font-bold underline underline-offset-2 hover:text-[#4C1D95] transition-colors"
          >
            Ver planos e preços
          </button>
        </div>
