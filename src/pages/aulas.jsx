...
import { 
  ChevronLeft, Play, Clock, CheckCircle, 
...
const Aulas = () => {
  ...
  const [visualizarPDF, setVisualizarPDF] = useState(false); // Estado para o iframe do PDF

  useEffect(() => {
    carregarConteudo();
  }, [cursoId]);

  // Reseta a visualização do PDF ao trocar de aula
  useEffect(() => {
  ...
  if (!temAcesso) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8 text-center">
      <div className="w-24 h-24 bg-red-50 text-red-500 rounded-[32px] flex items-center justify-center mb-8 shadow-inner"><Lock size={48} /></div>
      <h2 className="text-3xl font-black text-slate-900 mb-4 italic uppercase tracking-tighter">Área Restrita</h2>
      <a href={dadosCurso?.checkout_url || "#"} target="_blank" rel="noopener noreferrer" className="bg-[#5B2DFF] text-white px-12 py-5 rounded-3xl font-black text-xs uppercase shadow-2xl flex items-center gap-3">
      ...
      <header className={`sticky top-0 z-[100] border-b backdrop-blur-md transition-all ${modoCinema ? 'bg-black/80 border-white/5' : 'bg-white/80 border-slate-200/60'}`}>
        <div className="max-w-[1600px] mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/cursos" className={`p-2 rounded-xl transition-all ${modoCinema ? 'hover:bg-white/10 text-white' : 'hover:bg-slate-100 text-slate-600'}`}><ChevronLeft size={20} /></Link>
        ...
        <div className="flex-1 space-y-6">
          <div className={`relative group overflow-hidden shadow-2xl transition-all duration-700 ${modoCinema ? 'md:rounded-none' : 'md:rounded-[40px] rounded-none'} bg-black`}>
             <div className="aspect-video w-full">
                {aulaAtiva && (
                  <iframe className="w-full h-full" src={formatarVideoUrl(aulaAtiva.video_url)} title={aulaAtiva.titulo} frameBorder="0" allowFullScreen />
                )}
             </div>
          </div>
          ...
          <div className={`px-4 md:px-0 space-y-6 transition-opacity duration-500 ${modoCinema ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <button 
                ...
                className={`flex items-center justify-center gap-3 px-8 py-4 rounded-[24px] font-black text-xs transition-all shadow-xl ${
                  concluidas.has(aulaAtiva?.id) ? 'bg-green-500 text-white' : 'bg-[#5B2DFF] text-white'
                }`}
              >
            ...
            {/* --- NOVO CARD: MATERIAL DE APOIO PDF --- */}
            {aulaAtiva?.material_url && (
              <div className="bg-white rounded-[40px] border border-slate-200/60 shadow-sm overflow-hidden transition-all duration-500">
                ...
                {/* VISUALIZAÇÃO DO PDF ABAIXO DA AULA */}
                {visualizarPDF && (
                  <div className="p-4 border-t border-slate-100 bg-slate-50 animate-in slide-in-from-top duration-500">
                    <div className="aspect-[1/1.4] md:aspect-video w-full rounded-2xl overflow-hidden border border-slate-200 shadow-inner bg-white">
                       <iframe 
                        src={`${aulaAtiva.material_url}#toolbar=0`} 
                        className="w-full h-full" 
                        title="Material de Apoio"
                       />
                    </div>
                    <div className="mt-4 flex justify-center">
                       <a href={aulaAtiva.material_url} target="_blank" className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase hover:text-[#5B2DFF] transition-colors">
          ...
          <div className="bg-white rounded-[40px] border border-slate-200/60 shadow-sm overflow-hidden flex flex-col h-[700px]">
              ...
              <div className="space-y-2">
                {aulas.map((aula, idx) => (
                  <button 
                    ...
                    className={`w-full flex items-center gap-4 p-4 rounded-3xl transition-all ${aulaAtiva?.id === aula.id ? 'bg-slate-900 text-white' : 'hover:bg-slate-50 text-slate-700'}`}
                  >
                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${aulaAtiva?.id === aula.id ? 'bg-[#5B2DFF]' : concluidas.has(aula.id) ? 'bg-green-50 text-green-500' : 'bg-slate-100'}`}>
                      {concluidas.has(aula.id) ? <CheckCircle size={18} /> : <span className="text-[11px] font-black">{idx + 1}</span>}
                    </div>
                    <span className="text-[13px] font-bold truncate text-left">{aula.titulo}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {porcentagem === 100 && (
              <div className="p-6 bg-gradient-to-br from-yellow-400 to-orange-500 m-4 rounded-[32px] text-white text-center shadow-lg">
                ...
                <button onClick={gerarCertificado} className="mt-3 w-full py-3 bg-white text-orange-500 rounded-2xl font-black text-[10px] uppercase">Emitir Certificado</button>
              </div>
            )}
          </div>
        </aside>
