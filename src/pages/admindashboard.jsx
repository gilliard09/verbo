...
const CelebracaoMeta = ({ visivel, label, onFim }) => {
  ...
  useEffect(() => {
    if (!visivel) return;
    const cores = ['#4C1D95','#FFD700','#00C896','#FF3CAC','#fff','#a78bfa'];
...
const AulaItem = ({ aula, index, onDragStart, onDragOver, onDrop, onEditar, onDeletar, editando, onSalvarEdicao }) => {
  const [dadosEdicao, setDadosEdicao] = useState({titulo:aula.titulo,descricao:aula.descricao||'',video_url:aula.video_url});
  return (<div draggable onDragStart={()=>onDragStart(index)} onDragOver={(e)=>{e.preventDefault();onDragOver(index);}} onDrop={()=>onDrop(index)} className={`bg-white p-4 rounded-[20px] border transition-all ${editando?'border-[#4C1D95]/30 shadow-lg shadow-purple-50':'border-slate-100 hover:shadow-sm'}`}>{editando?(<div className="space-y-3"><input value={dadosEdicao.titulo} onChange={e=>setDadosEdicao(d=>({...d,titulo:e.target.value}))} className="w-full p-3 bg-slate-50 rounded-xl text-sm font-bold border-none focus:ring-2 focus:ring-purple-200 outline-none" placeholder="Título da aula"/><input value={dadosEdicao.video_url} onChange={e=>setDadosEdicao(d=>({...d,video_url:e.target.value}))} className="w-full p-3 bg-slate-50 rounded-xl text-sm border-none focus:ring-2 focus:ring-purple-200 outline-none" placeholder="URL do vídeo"/><textarea value={dadosEdicao.descricao} onChange={e=>setDadosEdicao(d=>({...d,descricao:e.target.value}))} className="w-full p-3 bg-slate-50 rounded-xl text-sm border-none focus:ring-2 focus:ring-purple-200 outline-none resize-y min-h-[100px]" placeholder="Descrição da aula" rows={4}/><div className="flex gap-2"><button onClick={()=>onSalvarEdicao(aula.id,dadosEdicao)} className="flex-1 py-2.5 bg-[#4C1D95] text-white rounded-xl font-black text-[10px] uppercase flex items-center justify-center gap-1.5"><Check size={12}/>Salvar</button><button onClick={()=>onEditar(null)} className="px-4 py-2.5 border border-slate-200 rounded-xl font-black text-[10px] uppercase text-slate-500">Cancelar</button></div></div>):(<div className="flex items-center gap-3"><div className="cursor-grab active:cursor-grabbing text-slate-200 hover:text-slate-400 transition-colors"><GripVertical size={18}/></div><div className="w-7 h-7 bg-slate-100 rounded-xl flex items-center justify-center shrink-0"><span className="text-[10px] font-black text-slate-400">{aula.ordem}</span></div><div className="flex-1 min-w-0 text-left"><span className="block text-sm font-bold text-slate-700 truncate">{aula.titulo}</span>{aula.descricao&&<span className="block text-[10px] font-medium text-slate-400 truncate mt-0.5">{aula.descricao}</span>}</div><div className="flex gap-1.5 shrink-0"><button onClick={()=>onEditar(aula.id)} className="p-2 bg-purple-50 text-[#4C1D95] rounded-xl hover:bg-[#54C1D95] hover:text-white transition-all"><Edit3 size={14}/></button><button onClick={()=>onDeletar(aula.id)} className="p-2 bg-red-50 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all"><Trash2 size={14}/></button></div></div>)}</div>);
};
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 py-2.5 bg-[#4C1D95] text-white rounded-xl font-black text-[10px] uppercase flex items-center justify-center gap-1.5"
            >
          ...
          <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <div className="flex gap-1.5 shrink-0">
              ...
              <button
                ...
                className="p-2 bg-purple-50 text-[#4C1D95] rounded-xl hover:bg-[#4C1D95] hover:text-white transition-all"
              >
                <Edit3 size={14} />
              </button>
              <button
